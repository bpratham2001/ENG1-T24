...
public class FreezeUp extends PowerUp {
     ...
     */
    public FreezeUp(final Vector2 position) {
        super(position);
        setRGB(Config.DEFAULT_FREEZEUP_RED, Config.DEFAULT_FREEZEUP_GREEN,
                Config.DEFAULT_FREEZEUP_BLUE);
    }
    ...
    public void update(final Player player) {
        ...
        if (isTaken() && !isUsed()) {
            ...
            if (timer < Config.FREEZEUP_TIME) {
                player.freezeUp(true);
            } else {
                player.freezeUp(false);
                setUsed(true);
            }
        }
    }
